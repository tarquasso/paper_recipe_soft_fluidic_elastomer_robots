...
%%
beta = 232; %Newtons / Amp
lambda = 475; %Volts / meter / sec
mp = 0.185; % kiligrams
Rm = 18.5; % Ohms
Ap = 0.00079; % meters^2
...
Lt = 0.4572; %meters
dt = 0.001016; %meters
Rt = 128*mu*Lt/(pi*dt^4); % Pascal / meter^3 / sec

a = [   -1*beta*lambda/(mp*Rm),      0,                 -Ap^2/mp; 
        0,                          ((Cf + Cc)*Rt)^-1,  -((Cf + Cc)*Rt)^-1;
        -1/Cf, -1/(Cf*Rt), 1/(Cf*Rt)];
...
a = [-1*beta*lambda/(mp*Rm), 0, -Ap^2/mp; 0, ((Cf + Cc)*Rt)^-1, -((Cf + Cc)*Rt)^-1; -1/Cf, -1/(Cf*Rt), 1/(Cf*Rt)];
b = [beta*Ap/(mp*Rm); 0; 0];
